    ...
    if not settings.openai_api_key or settings.openai_api_key == "mock-openai-key":
        ...
        try:
            ...
            system_prompt = (
                ...
                "- `industrial_analysis`: Industrial point-source outfall audit data (risk_score 0-100, risk_rating A-F, high_risk_facilities, chemical_signature_match, npdes_violations_summary, tri_releases_summary).\n"
                "- `agricultural_analysis`: Agricultural non-point source runoff audit data (risk_score 0-100, risk_rating A-F, crop_coverage %, cafos_in_watershed, eutrophication_index, nutrient_signature_match).\n"
                "- `attains_status`: EPA CWA Section 303(d) impairment records (assessment_unit_id, overall_status, impairment_causes, is_primary_path).\n"
                ...
                "- `industrial_weight_pct`: Calculated proportional contribution of industrial point-sources (float 0.0 to 100.0, industrial_weight_pct + agricultural_weight_pct = 100.0).\n"
                "- `agricultural_weight_pct`: Calculated proportional contribution of agricultural non-point sources (float 0.0 to 100.0).\n"
                "- `synthesis_reasoning`: Factual, authoritative scientific synthesis explaining the exact root causes behind the waterbody pollution, evidence correlation, and severity without referencing internal agent steps.\n"
                "- `remediation_recommendations`: List of actionable, prioritized mitigation steps (e.g. NPDES permit enforcement vs agricultural riparian buffers).\n"
