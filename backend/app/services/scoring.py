...
async def reason_and_score_with_openai(
    ...
    system_prompt = (
        ...
        "- `attains_status`: EPA CWA Section 303(d) impairment status (fields: assessment_unit_id, overall_status, impairment_causes, affected_uses, is_primary_path).\n"
        "- `polluters`: EPA ECHO NPDES permitted facilities (fields: facility_name, npdes_id, effluent_exceedances, quarters_in_noncompliance).\n"
        "- `water_quality_samples`: WQP chemical measurements (characteristic_name, result_value, unit_code).\n"
        ...
        "==================================================\n"
        "- Deduce overall categorical risk rating ('A' through 'F') and risk label ('Low Risk' through 'Critical Risk').\n"
        "- Infer risk-increasing factors (with evidence traceability like 'Persistent PCB impairment [ATTAINS, listed 2006]').\n"
        "- Infer risk-mitigating factors (e.g. Fully Supporting units, favorable chemistry, stable hydrology).\n"
        "- Infer temporal trend (Historical vs Persistent vs New vs Current) and spatial scope (Primary Path vs Contextual Watershed).\n"
        "- State data limitations and clear diagnostic notes.\n\n"
        "==================================================\n"
        "3. REASONING RULES & GUIDELINES\n"
        "==================================================\n"
        ...
        "  IMPORTANT: ATTAINS Assessment Units now include an 'is_primary_path' boolean. If true, this AU directly intersects the user's selected river path and should be the PRIMARY basis of your ATTAINS score. If false, it represents surrounding contextual watersheds/tributaries and should NOT directly penalize the main river unless clearly relevant.\n"
        ...
        "Spatially, distinguish whether impairment is localized to one Assessment Unit or widespread. Never blindly aggregate units. Focus heavily on AUs where is_primary_path=True.\n\n"
